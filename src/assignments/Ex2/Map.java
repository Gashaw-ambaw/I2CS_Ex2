...
public class Map implements Map2D, Serializable {
    ...
    public void init(int[][] arr) {
        ...
        for (int i = 0; i < h; i++) {                    //We run over the rows , y
            //Checking  that the arr is not  ragged
            if ((arr[i] == null || arr[i].length != w)) {
                throw new RuntimeException("Error: the array ragged at " + i);
            }
        }
        this._map = transpose(arr);
    }
    ...
    public int[][] getMap() {
        int w = getWidth();
        int h = getHeight();
        int[][] deepCopyMap = new int[h][w];

        for (int i = 0; i < w; i++) {
            for (int j = 0; j < h; j++) {
                deepCopyMap[j][i] = _map[i][j];
            }
        }
        return deepCopyMap;
    }
    ...
    public void addMap2D(Map2D p) {
        ...
        if (!sameDimensions(p)) {            //If it's not the same dim we return
            return;
        }
     ...
     //Multiplying the value of arrays by a scalar
    @Override
    ...
    public Map2D allDistance(Pixel2D start, int obsColor, boolean cyclic) {
        ...
        for (int i = 0; i < w; i++) {                // We ran over all the points in the
            for (int j = 0; j < h; j++) {           // disMap and we restart them , if its
                //point=obs==-2  else  -1.
        ...
        Queue<Pixel2D> q = new LinkedList<Pixel2D>();  //Create a queue
        q.add(start);                                  //and add the start point.
        ...
        return new Map(transpose(distMap));        //We do also transpose
    }
    ////////////////////// Private Methods ///////////////////////
    ...
    //The func swap the rows and the columns of the 2D array
    //needed because  in Java  its [row][col] (y,x)  ,but we use (x,y) , so that
    //func fix it.
    private int[][] transpose(int[][] matrix) {
        int w = matrix.length;
        int h = matrix[0].length;
        int[][] transpose = new int[h][w];
